...
import { 
    ContentElement, HeaderElement, ParagraphElement, ImageElement, 
    ListElement, CodeBlockElement, CustomComponentElement, InteractiveElement, 
    ErrorElement
} from './contentTypes';
...
export function renderContent(elements: ContentElement[]): (JSX.Element | null)[] {
    return elements.map((element, index) => {
        switch (element.type) {
            ...
            case 'paragraph':
                return <p key={index} dangerouslySetInnerHTML={{ __html: (element as ParagraphElement).content }} />;
            case 'image':
            ...
            case 'accordion':
                return <details key={index} className={element.style}><summary>Accordion Title</summary><p>{element.content}</p></details>;
            case 'tab':
                return <div key={index} className={element.style}><p>{element.content}</p></div>;
            case 'gallery':
                return <div key={index} className={element.style}>{element.content.split(';').map((url, idx) => <img key={idx} src={url} alt={`Gallery image ${idx}`} className="w-32 h-32" />)}</div>;
            case 'carousel':
                return <div key={index} className={element.style}>{element.content.split(';').map((url, idx) => <img key={idx} src={url} alt={`Carousel image ${idx}`} className="w-32 h-32" />)}</div>;
            case 'code':
                return <pre key={index} className={element.style}><code>{element.content}</code></pre>;
            case 'testimonial':
                return <div key={index} className={`p-4 shadow-lg rounded-lg ${element.style}`}><p>{element.content}</p></div>;
            case 'countdown':
                return <div key={index} className={element.style}><p>Countdown to: {element.content}</p></div>;
            case 'alert':
                return <div key={index} className={`p-4 shadow-lg rounded-lg ${element.style}`}><p>{element.content}</p></div>;
            case 'table':
                return (
                    <table key={index} className={`table-auto w-full ${element.style}`}>
                        <thead>
                            <tr>{element.content.split(';')[0].split(',').map((header, idx) => <th key={idx} className="px-4 py-2">{header}</th>)}</tr>
                        </thead>
                        <tbody>
                            {element.content.split(';').slice(1).map((row, idx) => (
                                <tr key={idx}>
                                    {row.split(',').map((cell, idx) => <td key={idx} className="border px-4 py-2">{cell}</td>)}
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            case 'progress':
                return (
                    <div key={index} className="w-full bg-gray-200 rounded-full dark:bg-gray-700">
                        <div className={`bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full ${element.style}`} style={{ width: `${element.content}%` }}>
                            {element.content}%
                        </div>
                    </div>
                );
                ...
                return (
                    <ol key={index} className={`list-decimal pl-5 ${element.style}`}>
                        {element.content.split(';').map((item, idx) => (
                            <li key={idx}>{item.split(':')[0]}: {item.split(':')[1]}</li>
                        ))}
                    </ol>
                );
            case 'youtube':
                return <iframe key={index} src={element.url} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className={`w-full h-64 ${element.style}`}></iframe>;
            case 'feature':
                return (
                    <div key={index} className={`p-4 shadow-lg rounded-lg ${element.style}`}>
                ...
                return (
                    <details key={index} className={`p-4 ${element.style}`}>
                        ...
                        {element.content.split(';').map((faq, idx) => {
                            ...
                            return <p key={idx}><strong>{parts[0]}</strong>: {parts[1]}</p>;
                        })}
                    </details>
                );
            ...
            case 'info-card':
                ...
                const linkTarget = cardParts[2].split(':')[1].trim() === 'true' ? '_blank' : '_self';
                return (
                    <div key={index} className={`p-4 shadow-lg rounded-lg ${element.style}`}>
                        <h3>{cardParts[0].split(':')[1]}</h3>
                        <p>{cardParts[1].split(':')[1]}</p>
                        <a href={cardParts[1].split(':')[1]} target={linkTarget} className="mt-2 inline-block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Visit</a>
                    </div>
                );
            case 'server-info':
                ...
                return (
                    <div key={index} className={`p-4 shadow-lg rounded-lg ${element.style}`}>
                        ...
                        <p>Server IP: <span>{ip}</span> <button onClick={() => navigator.clipboard.writeText(ip)} className="ml-2 text-blue-500 underline">Copy</button></p>
                        <a href={serverParts[2].split(':')[1].trim()} className="mt-2 inline-block bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Quick Join</a>
                    </div>
                );
            case 'admin-command':
                return (
            ...
            case 'textstyle':
                return <p key={index} className={element.style}>{element.content}</p>;
            case 'spacing':
                return <div key={index} className={element.style}>{element.content}</div>;
            default:
                return null;
        }
    });
